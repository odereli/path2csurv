...
source("clustered_multitask_multiple_kernel_survival_train.R")
source("clustered_multitask_multiple_kernel_survival_test.R")
...
cluster_count <- 5 #change if you would like to use different cluster counts
...
for (replication in 1:100) {
  if (file.exists(sprintf("%s/%s/cmtmkl_pathway_%s_measure_CI_cluster_count_%d_replication_%d_result.RData", result_path, full_name, pathway, cluster_count, replication)) == FALSE) {
    X <- vector("list", T)
    y <- vector("list", T)
    delta <- vector("list", T)
    ...
    C_set <- c(10, 100, 1000, 10000, 100000)
    epsilon <- 1e-3
    fold_count <- 4
    train_ratio <- 0.8
    iteration_count <- 150
    ...
    for (fold in 1:fold_count) {
      ...
      print(sprintf("replication = %g", replication))
      for (C in C_set) {
        print(sprintf("running fold = %d, C = %g, cluster_count = %g", fold, C, cluster_count))
        parameters <- list()
        parameters$C <- C
        ...
        state <- clustered_multitask_multiple_kernel_survival_train(K_train, y_train, delta_train, parameters)
        prediction <- clustered_multitask_multiple_kernel_survival_test(K_test, state)
        for (t in 1:T) {
    ...
    save("state", file = sprintf("%s/%s/cmtmkl_pathway_%s_measure_CI_cluster_count_%d_replication_%d_state.RData", result_path, full_name, pathway, cluster_count, replication))
    save("result", file = sprintf("%s/%s/cmtmkl_pathway_%s_measure_CI_cluster_count_%d_replication_%d_result.RData", result_path, full_name, pathway, cluster_count, replication))
  }
}
